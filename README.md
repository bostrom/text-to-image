...
## Usage
    ...
    textToImage.generate('Lorem ipsum dolor sit amet').then(function (dataUrl) {
      console.log(dataUrl);
    });

The above script will print the full generated data URL to the console.
...
Turn on debug mode to have the library store the generated image data URL as a PNG in the current working directory (where the process was started). The image name will be the current date and time.
